...
var _ndnBuyerId = 'Richmond_Times_Dispatch_369';
var _matchers = [];
var placements;
var distributorId = '91348';
       ...
       if (pardiv) {
         if(unitsdata[i].render_app=='widget'){
           if (NDNAnalytics.hasFlash){
             ...
             writeWidget(unitsdata[i].target);
		...
		if(!pardiv.getAttribute('data-pp-type')){
             if(unitsdata[i].ndn_vid_id != ''){
		    pardiv.setAttribute('data-pp-type', unitsdata[i].render_app);
		    var el = NDNRender.createElement("iframe", {"src": unitsdata[i].url, "height": unitsdata[i].height , 
		           "width": unitsdata[i].width, "scrolling": "no", "frameborder": "no", "noresize": "noresize",
		    ...
		    NDNRender.beacon( {
              ...
              if(unitsdata[i].render_app == 'blank'){
                pardiv.setAttribute('data-pp-type', unitsdata[i].render_app);
              }
            }
		}
...
var NDNDynVideoWidgets = function() {
  ...
  if(_matchers.length === 0){
    ...
    + "/repub/" + _ndnBuyerId + "/unitsdata.js?dpid=" + distributorId + "&loc=" + encodeURIComponent(NDNRender.url) + "&matchers="
